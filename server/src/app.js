...
function createServer({ dataDirectory = path.join(__dirname, '..', 'data') } = {}) {
  ...
  return http.createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === 'POST' && pathname === '/api/v1/cod/calculate') {
        return sendJson(response, 200, { data: calculateCOD(validateCalculation(await readJson(request))) });
      }
      if (request.method === 'POST' && pathname === '/api/v1/cod/average') {
        const input = validateAverage(await readJson(request));
        return sendJson(response, 200, { data: calculateAverage(input.values, input.range) });
      }
      if (request.method === 'POST' && pathname === '/api/v1/measurements') {
        const input = validateCalculation(await readJson(request));
      ...
      if (request.method === 'GET' && pathname === '/api/v1/measurements') {
        return sendJson(response, 200, { data: await store.list() });
      }
...
if (require.main === module) {
  ...
  createServer().listen(port, () => console.log(`COD service listening on port ${port}`));
}
